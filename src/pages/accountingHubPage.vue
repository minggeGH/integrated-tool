        ...
        <div class="rounded-[32px] bg-gradient-to-br from-primary to-info p-4 text-white shadow-2xl sm:p-5">
          <div class="flex items-start justify-between gap-3">
            <div>
              ...
              <div class="mt-1 text-3xl font-bold tracking-tight sm:text-4xl">{{ countResultDisplay }}</div>
          ...
          <div class="mt-4 grid gap-2 sm:grid-cols-3">
            <div class="rounded-2xl border px-3 py-3 shadow-lg backdrop-blur" :class="heroMetricClass">
              <div class="text-xs uppercase tracking-[0.2em] text-white/70">余额</div>
              <div class="mt-2 text-2xl font-semibold">{{ balanceDisplay }}</div>
            </div>
            <div class="app-kpi bg-white text-slate-700">
              <div class="text-xs uppercase tracking-[0.2em] text-white/70">扣除合计</div>
              <div class="mt-2 text-2xl font-semibold">{{ sumSubtract }}</div>
            </div>
            <div class="app-kpi bg-white text-slate-700">
              <div class="text-xs uppercase tracking-[0.2em] text-white/70">伙食总计</div>
              <div class="mt-2 text-2xl font-semibold">{{ totalFoodExpense }}</div>
            </div>
          </div>
        </div>
      ...
      <section class="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <div class="app-surface p-5">
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            ...
            <div v-if="!subtractItems.length" class="w-full rounded-3xl border border-dashed border-base-300 px-4 py-8 text-center text-sm text-base-content opacity-60">
              还没有扣除项，先从右侧表单加一条
            </div>
          </div>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <q-btn
                  ...
                  no-caps
                  size="sm"
                  :color="selectedTag === tag ? 'primary' : 'grey-3'"
...
const sumSubtract = computed(() =>
  subtractItems.value.reduce((sum, item) => sum + (Number(item.value) || 0), 0)
);
...
const countResultDisplay = computed(() => `${countResult.value}`);
...
function onAddItem() {
  ...
  subtractItems.value.push({ tag, value: amount });
  subtractInput.value = "";
  customTag.value = "";
